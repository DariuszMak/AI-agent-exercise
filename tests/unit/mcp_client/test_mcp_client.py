...
def test_list_tools_returns_tools() -> None:
...
def test_list_tools_raises_on_connection_error() -> None:
    client = MCPClient(server_url="http://localhost:8765")
    with (
        patch("requests.post", side_effect=requests.ConnectionError("refused")),
        pytest.raises(ConnectionError, match="MCP server unavailable"),
    ):
        client.list_tools()
...
def test_call_tool_is_error_true() -> None:
    client = MCPClient(server_url="http://localhost:8765")
...
def test_call_tool_on_connection_error_returns_error_result() -> None:
    client = MCPClient(server_url="http://localhost:8765")
...
def test_call_tool_on_value_error_returns_error_result() -> None:
    client = MCPClient(server_url="http://localhost:8765")
...
def test_call_tool_multiple_text_blocks_joined() -> None:
    client = MCPClient(server_url="http://localhost:8765")
    content = [
...
def test_call_tool_ignores_non_text_blocks() -> None:
    client = MCPClient(server_url="http://localhost:8765")
...
def test_rpc_raises_value_error_on_json_rpc_error() -> None:
    client = MCPClient(server_url="http://localhost:8765")
    with (
...
def test_rpc_raises_connection_error_on_http_error() -> None:
    client = MCPClient(server_url="http://localhost:8765")
    resp = MagicMock()
...
def test_rpc_returns_empty_dict_for_non_dict_result() -> None:
    client = MCPClient(server_url="http://localhost:8765")
    resp = MagicMock()
    resp.raise_for_status.return_value = None
...
def test_rpc_sends_correct_headers() -> None:
    client = MCPClient(server_url="http://localhost:8765")
...
def test_session_id_consistent_across_calls() -> None:
    client = MCPClient(server_url="http://localhost:8765")
    captured: list[str] = []
    with patch("requests.post", return_value=_rpc_response({})) as mock_post:
        client._rpc("tools/list", {})
...
def test_extracts_text_from_list() -> None:
    content = [{"type": "text", "text": "hello"}]
    assert _extract_text(content) == "hello"


def test_joins_multiple_texts() -> None:
...
def test_non_list_returns_str() -> None:
