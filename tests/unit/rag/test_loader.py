...
def test_load_documents_returns_chunks(tmp_path: Path) -> None:
    ...
    docs.mkdir()
    (docs / "empire_state_building.txt").write_text("KSeF to Krajowy System e-Faktur.", encoding="utf-8")

    documents = load_documents(docs)

    assert len(documents) > 0
    assert documents[0]["id"] == "empire_state_building.txt"
    assert "Empire State Building" in documents[0]["text"]
