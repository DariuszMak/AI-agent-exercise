...
from src.rag.api.llm import LLMResponse, OllamaAdapter
...
def test_chat_strips_content() -> None:
    adapter = OllamaAdapter()
...
def test_chat_missing_content_key() -> None:
    adapter = OllamaAdapter()
...
def test_chat_missing_model_key_falls_back() -> None:
...
def test_chat_done_defaults_to_true() -> None:
    adapter = OllamaAdapter()
...
def test_post_raises_connection_error_on_request_exception() -> None:
    adapter = OllamaAdapter()
    with (
...
def test_post_raises_on_non_dict_response() -> None:
    adapter = OllamaAdapter()
    resp = MagicMock()
    resp.raise_for_status.return_value = None
...
def test_post_uses_correct_url() -> None:
...
def test_post_sends_stream_false() -> None:
    adapter = OllamaAdapter()
    body = {"message": {"content": "ok"}, "model": "x", "done": True}
    with patch("requests.post", return_value=_make_response(body)) as mock_post:
        adapter.complete("q")
